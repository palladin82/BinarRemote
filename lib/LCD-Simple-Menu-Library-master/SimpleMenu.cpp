...
SimpleMenu::SimpleMenu(int _numberMenuItems, SimpleMenu *_submenus) //default
{
 numberMenuItems = _numberMenuItems;
 submenus = _submenus;
}

SimpleMenu::SimpleMenu(void (*_CallBack)()) //default
{
  CallBack = _CallBack;
}

SimpleMenu::SimpleMenu(String _name, void (*_CallBackP)(int *_param), int _param)
{  
  CallBackP = _CallBackP;
  name = _name;  
  param = _param;
}
...
SimpleMenu::SimpleMenu(String _name, uint8_t *_value, int _min, int _max) //Value menu
{
  name = _name;
  uvalue = _value;
  min = _min;
